...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return res.status(resp.status).json({ error: `Delete failed (${resp.status})`, details: errText.slice(0, 200) });
    }
    return res.status(200).json({ success: true });
  } catch (e) {
    console.error('Delete-file error:', e);
    return res.status(500).json({ error: 'Failed to delete file', details: e?.message || String(e) });
  }
}