...
//
// Upload a document to the OpenAI file store and attach it to your vector
// store. Accepts multipart/form-data using a "document" or "file" field.
// Adds CORS + robust multipart parsing for Vercel Node runtimes.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!uploadResp.ok || !uploaded?.id) {
      return res.status(uploadResp.status || 500).json({ success: false, error: 'File upload failed', details: uploaded });
    }

    // 2) Attach to Vector Store
    // Using the simple attach endpoint for broad compatibility.
    ...
    if (!attachResp.ok || attached?.error) {
      return res.status(attachResp.status || 500).json({ success: false, error: 'Attach failed', details: attached });
    }
