...
// Works on Vercel Node runtime with large uploads (no extra deps).

export const config = {
  api: {
    bodyParser: false,   // we'll parse multipart ourselves
    sizeLimit: '50mb',   // raise as needed
  },
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // --- naive multipart parse (single file field: "document" or "file") ---
    ...
    if (!file_id) {
      return res.status(500).json({ success: false, error: 'Unexpected upload response', details: uploadText.slice(0, 500) });
    }
    ...
    if (!attachResp.ok) {
      return res.status(attachResp.status).json({
        ...
        details: attachText.slice(0, 500),
      });
    }
