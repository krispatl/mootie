// api/vector-store.js
// Lists files currently attached to your vector store (v1) and resolves filenames.
  ...
  try {
    ...
    if (!listResp.ok) {
      return res.status(listResp.status).json({
        success: false,
        error: 'Vector list failed',
        details: listText.slice(0, 500),
      });
    }

    let data;
    try { data = JSON.parse(listText); } catch { data = { data: [] }; }
    const items = Array.isArray(data?.data) ? data.data : [];

    // Resolve filenames/bytes/created_at via /v1/files/{id}
